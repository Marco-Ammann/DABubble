  ...
  private async addUserToDeveloperTeamChannel(userId: string): Promise<void> {
    ...
    if (!guestChannelObj) {
      guestChannelObj = {
        ...
        isPrivate: false,
        description:
          'Dieser Channel ist für Gäste vorgesehen. Hier können Gäste Fragen stellen und Feedback geben.',
        members: [],
        createdBy: userId,
        createdAt: new Date(),
        updatedAt: new Date(),
      ...
      const channelRef = await this.channelService.addChannel(
        guestChannelObj as NewChannel
      );
      guestChannelObj.id = channelRef.id;
    }
    ...
    if (!guestChannelObj.members.includes(userId)) {
      await this.channelService.updateChannel(guestChannelObj, {
        members: [...guestChannelObj.members, userId],
        updatedAt: new Date(),
      });
    }
  }
